...
*Setup instructions for a Gitian build of PAIcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PAIcoin
...
- [Building PAIcoin Core](#building-paicoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-paicoin-host/README.md).
...
Clone the git repositories for paicoin and Gitian.
...
in the paicoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building PAIcoin Core
----------------

To build PAIcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the paicoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/paicoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/paicoin/paicoin
...
For example:
```bash
URL=https://github.com/laanwj/paicoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit paicoin=${COMMIT} --url paicoin=${URL} ../paicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit paicoin=${COMMIT} --url paicoin=${URL} ../paicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit paicoin=${COMMIT} --url paicoin=${URL} ../paicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the paicoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../paicoin/contrib/gitian-descriptors/*|sort|uniq )
...
PAIPATH=/some/root/path/paicoin
SIGPATH=/some/root/path/paicoin-detached-sigs

./bin/gbuild --url paicoin=${PAIPATH},signature=${SIGPATH} ../paicoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/paicoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/paicoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/paicoin-osx-build.assert
```
...
[paicoin-core/gitian.sigs](https://github.com/paicoin-core/gitian.sigs/) repository, or if that's not possible create a pull

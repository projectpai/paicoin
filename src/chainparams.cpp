...
static CBlock MineGenesisBlock(uint32_t nTime, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward, const char* signature)
{
    ...
    genesis = CreateGenesisBlock(nTime, 0, nBits, 4, genesisReward, signature);

    arith_uint256 powLimit;
    powLimit.SetCompact(nBits);

    LogPrintf("Generating genesis block.");

    for (genesis.nNonce = 0; UintToArith256(genesis.GetHash()) > powLimit; genesis.nNonce++)
    {
        if (genesis.nNonce % 1024 == 0)
            LogPrintf(".");
    }

    LogPrintf("\nFound genesis block: %s\n", genesis.ToString());
...
public:
    CRegTestParams() {
        ...
        // NOTE PAICOIN Do not modify the BIP settings, otherwise the current txvalidationcache_tests will fail

...
std::vector<CAmount> CalculateNetRemunerations(const std::vector<TicketContribData>& contributions, const CAmount& stake, const CAmount& subsidy, const CAmount& fee, const FeeDistributionPolicy feePolicy)
{
    ...
    arith_uint256 funding(static_cast<uint64_t>(stake + subsidy));

    arith_uint256 arithFee(static_cast<uint64_t>(fee));
    CAmount paidFee{0}; // this is used for calculating and assigning rounding errors to the first contributor
    ...
    std::sort(indexes.begin(), indexes.end(), [&](const unsigned int a, const unsigned int b) {
        if (vote) {
            if (contributions[a].voteFees == TicketContribData::NoFees)
                return false;
            if (contributions[b].voteFees == TicketContribData::NoFees)
                return true;
            return contributions[a].voteFees > contributions[b].voteFees;
        } else {
            if (contributions[a].revokeFees == TicketContribData::NoFees)
                return false;
            if (contributions[b].revokeFees == TicketContribData::NoFees)
                return true;
            return contributions[a].revokeFees > contributions[b].revokeFees;
        }
    });
    ...
    // any rounding errors will be applied to the most generous contributor.
    ...
    for (unsigned i = 0; i < indexes.size(); ++i) {
        ...
        if (vote && contribution.hasVoteFeeLimits()) {
            if (contributionFee > contribution.voteFeeLimits())
                contributionFee = contribution.voteFeeLimits();
        }
        if (!vote && contribution.hasRevokeFeeLimits()) {
            if (contributionFee > contribution.revokeFeeLimits())
                contributionFee = contribution.revokeFeeLimits();
        }
        ...
        // obviously, for the first contributor policy, just abort.
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckCoinbase, int blockHeight)
{
    ...
    if (blockHeight < consensusParams.nStakeValidationHeight) {
        ...
        if (block.nVoteBits != VoteBits(VoteBits::Rtt, true))   // before stake validation height, blocks must all have voteBits set to 1 (simple approval)

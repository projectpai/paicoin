...
static void BuildAddrIndex(const CScript &script, const CExtDiskTxPos &pos, std::vector<std::pair<uint160, CExtDiskTxPos> > &out)
{
            ...
            if (!tx.IsCoinBase()) {
                for (const CTxIn& txin : tx.vin) {
                    ...
                    if (!view.GetCoin(txin.prevout, coin))
                        return error("ConnectBlock(): Missing input");

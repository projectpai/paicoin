...
void PAIcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a PAIcoin address"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible PAIcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void PAIcoinGUI::interruptForPinRequest(bool newPin)
{
    ...
    if (previousState == PAIcoinGUIState::Init || previousState == PAIcoinGUIState::PaperKeyCompletion)
    {
        ...
        disconnect(walletFrame, SIGNAL(requestedSyncWarningInfo()), this, SLOT(showModalOverlay()));
        disconnect(labelBlocksIcon, SIGNAL(clicked(QPoint)), this, SLOT(showModalOverlay()));
        disconnect(progressBar, SIGNAL(clicked(QPoint)), this, SLOT(showModalOverlay()));
    }
...
void PAIcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to PAIcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
static bool ThreadSafeMessageBox(PAIcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void PAIcoinGUI::subscribeToCoreSignals()
{
...
void PAIcoinGUI::unsubscribeFromCoreSignals()
{

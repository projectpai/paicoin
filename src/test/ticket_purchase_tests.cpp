...
public:
    ...
    std::vector<uint256> AddVoteTxs()
    {
        ...
        for (auto ticketHash: chainActive.Tip()->pstakeNode->Winners()) {
            ...
            const CAmount reward = CalculateGrossRemuneration(ticketContribData.contributedAmount, ticketPrice, voterSubsidy, ticketContribData.contributedAmount);
            ...
            VoteData voteData{1, hash, static_cast<uint32_t>(height), 0x0001, 0, ExtendedVoteBits()};

...
{
public:
    std::vector<CTransactionRef> coinbaseTxns;

    std::shared_ptr<CBlock> CreateBlock(const std::vector<CTransactionRef>& txns, const CScript& scriptPubKey);

    bool ProcessBlock(const std::shared_ptr<CBlock>& block);
...
std::shared_ptr<CBlock> DriveChainSetup::CreateBlock(const std::vector<CTransactionRef>& txns, const CScript& scriptPubKey)
{
    std::unique_ptr<CBlockTemplate> pblocktemplate = BlockAssembler(Params()).CreateNewBlock(scriptPubKey);
    std::shared_ptr<CBlock> block = std::make_shared<CBlock>(pblocktemplate->block);
...
bool DriveChainSetup::ProcessBlock(const std::shared_ptr<CBlock>& block)
{
    const CChainParams& chainparams = Params();

    unsigned int extraNonce = 0;
    IncrementExtraNonce(block.get(), chainActive.Tip(), extraNonce);

    while (!CheckProofOfWork(block->GetHash(), block->nBits, chainparams.GetConsensus()))
        ++block->nNonce;

    bool result = ProcessNewBlock(chainparams, block, true, NULL);
...
CTransactionRef CreateTxVote(const std::vector<unsigned char>& script)
{
...
std::map<int, CTransactionRef> CreateTxVote(int from, int to, const std::vector<unsigned char>& script)
{
    std::map<int, CTransactionRef> result;
...
class DriveChainTestCheckerBlockReader : public TransactionSignatureChecker, public BaseBlockReader
{
    ...
    std::map<int, CTransactionRef> txs;
...
public:
    ...
    DriveChainTestCheckerBlockReader(int blockNumber, const std::vector<unsigned char>& hashSpend, const std::map<int, CTransactionRef>& txs, const CAmount& amount)
...
public:
    ...
    template <typename T> SHA256Writer& operator << (const T& obj)
    {
        ::Serialize(*this, obj);
        return *this;
    }
...
void RunEvalScriptTest(std::vector<unsigned char> hash, std::map<int, CTransactionRef> txs, CScript scriptPubKey, int blockNumber, ScriptError result, int positive = -1, int negative = -1)
{
...
void RunVerifyScriptTest(std::vector<unsigned char> hash, std::map<int, CTransactionRef> txs, CScript witscript, int blockNumber, ScriptError result)
{
...
BOOST_AUTO_TEST_CASE(EvalScriptTest)
{
...

    {
        ...
        RunEvalScriptTest(hash, std::map<int, CTransactionRef>(), scriptPubKey, 200, SCRIPT_ERR_COUNT_ACKS_INVALID_PARAM);
    }
    {
        std::map<int, CTransactionRef> txs;
    ...
    }
    {
        ...
        RunEvalScriptTest(hash, std::map<int, CTransactionRef>(), scriptPubKey, 370, SCRIPT_ERR_COUNT_ACKS_INVALID_PARAM);
    }
    {
        std::map<int, CTransactionRef> txs;
        InsertMap(txs, CreateTxVote(101, 101, SerializeDrivechain(FullAckList() << ChainAckList(ChainIdFromString("XCOIN")) << Ack(ParseHex(""), preimage), true)));
...
BOOST_AUTO_TEST_CASE(VerifyScriptTest)
{
    ...
    {
        // Invalid block number
        RunVerifyScriptTest(hash, std::map<int, CTransactionRef>(), witscript, 250, SCRIPT_ERR_COUNT_ACKS_INVALID_PARAM);
    }
...
BOOST_AUTO_TEST_CASE(BlockchainTest)
{
    ...
    for (int i = 1; i <= 100 + BLOCK_BASE; ++i) {
        auto block = CreateBlock(std::vector<CTransactionRef>(), scriptPubKey);
    ...
    {
        const CChainParams& chainparams = Params();
        auto block = CreateBlock(std::vector<CTransactionRef>{MakeTransactionRef(std::move(spendTx))}, scriptPubKey);

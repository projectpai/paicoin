...
public:
    ...
    uint256 AddVoteTx(const uint256& ticketHash)
    {
        ...
        VoteData voteData{1, hash, static_cast<uint32_t>(height), VoteBits::rttAccepted, 0, defaultEvb};
        CScript declScript = GetScriptForVoteDecl(voteData);
        mtx.vout.push_back(CTxOut(0, declScript));

        for (const TicketContribData& contrib: contributions) {
            const CAmount reward = CalcContributorRemuneration(contrib.contributedAmount, ticketPrice, voterSubsidy, contributionSum);
...
BOOST_FIXTURE_TEST_CASE(vote_transaction, VoteTestingSetup)
{
    ...
    {
        // ticket hash
        std::tie(voteHashStr, we) = wallet->Vote(uint256(), chainActive.Tip()->GetBlockHash(), chainActive.Tip()->nHeight, VoteBits::allRejected, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::INVALID_PARAMETER);

        // stake validation height not reached yet
        std::tie(voteHashStr, we) = wallet->Vote(chainActive.Tip()->GetBlockHash(), chainActive.Tip()->GetBlockHash(), chainActive.Tip()->nHeight, VoteBits::allRejected, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::INVALID_PARAMETER);

        ExtendChain(consensus.nStakeValidationHeight - chainActive.Height());

        // block hash
        std::tie(voteHashStr, we) = wallet->Vote(chainActive.Tip()->GetBlockHash(), uint256(), chainActive.Tip()->nHeight, VoteBits::allRejected, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::INVALID_PARAMETER);
        std::tie(voteHashStr, we) = wallet->Vote(chainActive.Tip()->GetBlockHash(), chainActive.Tip()->pprev->GetBlockHash(), chainActive.Tip()->nHeight, VoteBits::allRejected, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::INVALID_PARAMETER);

        // block height
        std::tie(voteHashStr, we) = wallet->Vote(chainActive.Tip()->GetBlockHash(), chainActive.Tip()->GetBlockHash(), chainActive.Tip()->nHeight - 1, VoteBits::allRejected, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::INVALID_PARAMETER);
        std::tie(voteHashStr, we) = wallet->Vote(chainActive.Tip()->GetBlockHash(), chainActive.Tip()->GetBlockHash(), chainActive.Tip()->nHeight + 1, VoteBits::allRejected, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::INVALID_PARAMETER);
...

    {
        ...
        std::tie(voteHashStr, we) = wallet->Vote(ticketHash, chainActive.Tip()->GetBlockHash(), chainActive.Tip()->nHeight, VoteBits::rttAccepted, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::SUCCESSFUL);
...
BOOST_FIXTURE_TEST_CASE(foreign_ticket_vote_transaction, VoteTestingSetup)
{
    ...
    for (auto& hash : chainActive.Tip()->pstakeNode->Winners()) {
        std::tie(txHash, we) = wallet->Vote(hash, chainActive.Tip()->GetBlockHash(), chainActive.Tip()->nHeight, VoteBits::allRejected, defaultEvb);
        BOOST_CHECK_EQUAL(we.code, CWalletError::INVALID_ADDRESS_OR_KEY);
    }
}
...
BOOST_FIXTURE_TEST_CASE(generate_vote_rpc, VoteTestingSetup)
{
    ...
    CheckVote(voteHash, ticketHash, VoteData{1, blockHash, static_cast<uint32_t>(blockHeight), VoteBits::rttAccepted, 0, defaultEvb});
    ...
    CheckVote(voteHash, ticketHash, VoteData{1, blockHash, static_cast<uint32_t>(blockHeight), VoteBits::allRejected, 0, defaultEvb});
    ...
    voteHash = uint256S(find_value(r.get_obj(), "hex").get_str());
    CheckVote(voteHash, ticketHash, VoteData{1, blockHash, static_cast<uint32_t>(blockHeight), VoteBits(65535U), 0, defaultEvb});
    ...
    voteHash = uint256S(find_value(r.get_obj(), "hex").get_str());
    CheckVote(voteHash, ticketHash, VoteData{1, blockHash, static_cast<uint32_t>(blockHeight), VoteBits::rttAccepted, 0, ExtendedVoteBits(validLargeEvbString)});

...
- double proof attack
- self merged mining attack with parent submitted first and merged
  mined block second
- self merged mining attack with merged mined block submitted first
  and parent block second
- merged mining auxpow depth check
- invalid merkle branch

All the cases are testing both legacy and lean auxpow implementations"""
...
from test_framework.mininode import CBlock, CBlockHeader, CAuxPow, CMerkleTx, hash256, ToHex, uint256_from_str
...
class AuxPowAttackTest(PAIcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.test_doubleProofAttack_leanAuxpow()
        self.test_selfMergeAttack_Parent_Merged()
        self.test_selfMergeAttack_Parent_Merged_leanAuxpow()
        self.test_selfMergeAttack_Merged_Parent()
        self.test_selfMergeAttack_Merged_Parent_leanAuxpow()
        self.test_selfMergeAttack_Parent_Merged_WithDepth()
        self.test_selfMergeAttack_Parent_Merged_WithDepth_leanAuxpow()
        self.test_invalidMerkleBranch()
        self.test_invalidMerkleBranch_leanAuxpow()
    ...
    def test_doubleProofAttack_leanAuxpow(self):
        ...
        combined_hash = uint256_from_str(hash256(hash_block1 + hash_block2))

        # create parent block of auxpow attack, solve, submit
        node_parent_chain.generate(1)
        parent_blk_tmpl = node_parent_chain.getblocktemplate()
        coinbase_tx = self.createCoinbaseWithSequence(height=int(parent_blk_tmpl["height"]) + 1)

        parent_block = self.createBlockFromTemplate(block_template=parent_blk_tmpl, coinbase_tx=coinbase_tx)
        parent_block.auxpowChildrenHash = combined_hash
        parent_block.solve()

        result = node_parent_chain.submitblock(ToHex(parent_block))
        assert result is None

        # Now, build valid lean auxpow information to submit to child chain
        child_block1.auxpow = CAuxPow.fromData(None, [child_block2.sha256], 0, parent_block)
        result = node_child_chain.submitblock(ToHex(child_block1))
        assert result is None

        child_block2.auxpow = CAuxPow.fromData(None, [child_block1.sha256], 1, parent_block)
        result = node_child_chain.submitblock(ToHex(child_block2))
        assert 'auxpow-double-proof' in result
    ...
    def test_selfMergeAttack_Parent_Merged_leanAuxpow(self):
        ...
        coinbase_tx = self.createCoinbaseWithSequence(height=int(block_template["height"]) + 1)
        parent_block = self.createBlockFromTemplate(block_template=block_template, coinbase_tx=coinbase_tx)
        parent_block.auxpowChildrenHash = merged_mined_block.sha256
        parent_block.solve()
        ...
        # Now, build valid auxpow information to try to submit to the chain
        merged_mined_block.auxpow = CAuxPow.fromData(None, [], 0, parent_block)
    ...
    def test_selfMergeAttack_Parent_Merged_WithDepth_leanAuxpow(self):
    ...
    def test_selfMergeAttack_Merged_Parent_leanAuxpow(self):
    ...
    def test_invalidMerkleBranch_leanAuxpow(self):
        ...
        child_block1.auxpow = CAuxPow.fromData(None, [child_block2.sha256], 1, parent_block)
        result = node_child_chain.submitblock(ToHex(child_block1))
        assert 'high-hash' in result
        ...
        child_block2.auxpow = CAuxPow.fromData(None, [child_block1.sha256], 0, parent_block)
        result = node_child_chain.submitblock(ToHex(child_block2))
        assert 'high-hash' in result

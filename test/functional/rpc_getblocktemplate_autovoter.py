...
class TestGetBlockTemplate(PAIcoinTestFramework):
    ...
    def mine_using_template(self, nodeidx = 0):
        tmpl = self.get_block_template(nodeidx)
    ...
    def run_test(self):
        ...
        # for blkidx in range(nStakeValidationHeight + 1):
        #     self.nodes[blkidx % self.num_nodes].generate(1) # mine on each of the nodes, so also the seconds one has funds
        # self.sync_all()
        # self.get_best_block(nStakeValidationHeight +1)

        self.nodes[0].generate(1000)

        for idx in range(1000 + 1,nStakeValidationHeight + 1):
            # gbt_txids = self.get_block_template()
            self.mine_using_template(idx % self.num_nodes)

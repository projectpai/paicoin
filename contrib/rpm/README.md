...
The RPM spec file provided here is for PAIcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://paicoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[PAIcoin git repository](https://github.com/paicoin/paicoin) but are not part of
...
Currently only `Source10` is used, for the example `paicoin.conf` file.
...
in the paicoin git repository. At present only an SVG version of the PAIcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the paicoin
...
needed when 0.12.1 is released, a proper fix is already in the PAIcoin git
master branch.
...
idea to build PAIcoin Core against a static version of that library compiled
...
The `paicoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `paicoin-server` package. At this point, most RPM based
...
The `paicoin-server` package should have SELinux support. How to properly do
...
PAIcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/paicoin/). He has been
packaging PAIcoin for Fedora at least since 2012.
...
4. I renamed the `paicoin` package that contains the Qt GUI to `paicoin-core` as
that appears to be how the general population refers to it, in contrast to
`paicoin-xt` or `paicoin-classic`. I wanted to make sure the general population

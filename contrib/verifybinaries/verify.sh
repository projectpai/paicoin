...
###   paicoincore.org and paicoin.org and compares them.
...
WORKINGDIR="/tmp/paicoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://paicoincore.org"
HOST2="https://paicoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="paicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'paicoin-',
...
if [ $? -ne 0 ]; then
   echo "paicoin.org failed to provide signature file, but paicoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "paicoin.org and paicoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the PAIcoin Core binary release signing key installed?"
   fi
